 ...
 *   getPolynom()      => null
 */
function getPolynom(...polynomArguments) {
  return (x) => {
    switch (polynomArguments.length) {
      case 3: {
        const [a, b, c] = polynomArguments;

        return a * x ** 2 + b * x + c;
      }
      case 2: {
        const [b, c] = polynomArguments;

        return b * x + c;
      }
      case 1: {
        const [c] = polynomArguments;
...
function logger(func, logFunc) {
  return (...innerArgs) => {
    const signatureArgs = innerArgs.map(JSON.stringify).join(',');
    const functionSignature = `${func.name}(${signatureArgs})`;

    logFunc(`${functionSignature} starts`);

    const result = func.apply(this, innerArgs);

    logFunc(`${functionSignature} ends`);
...
function partialUsingArguments(fn, ...args1) {
  return (...innerArgs) => {
    return fn.apply(this, [...args1, ...innerArgs]);
  };
}
